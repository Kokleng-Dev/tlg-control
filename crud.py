...
async def upsert_chat(session: AsyncSession, bot: Bot, chat_obj: dict) -> Chat:
    ...
    if chat is None:
        chat = Chat(
            ...
            title=chat_obj.get("title") or chat_obj.get("username"),
...
async def upsert_membership(session: AsyncSession, bot: Bot, chat: Chat, user: User, status: str = "member", role: str = "member"):
    q = await session.execute(select(Membership).where(Membership.bot_id == bot.id, Membership.chat_id == chat.id, Membership.user_id == user.id))
    membership = q.scalars().first()
    now = datetime.now(timezone.utc)
    if membership is None:
        membership = Membership(bot_id=bot.id, chat_id=chat.id, user_id=user.id, status=status, role=role, joined_at=now)
        session.add(membership)
    else:
        # Update role/status and timestamps
...
async def log_action(session: AsyncSession, bot: Bot, chat: Optional[Chat], user_telegram_id: Optional[int], action: str, reason: Optional[str]=None, payload: Optional[str]=None):
    entry = ActionLog(bot_id=bot.id, chat_id=(chat.id if chat else None), user_telegram_id=user_telegram_id, action=action, reason=reason, payload=payload)
    session.add(entry)
    await session.commit()
...
async def list_members_in_chat(session: AsyncSession, bot: Bot, chat_telegram_id: int):
    q = await session.execute(select(Membership).join(Chat).where(Membership.bot_id == bot.id, Chat.telegram_chat_id == chat_telegram_id))
    return q.scalars().all()
